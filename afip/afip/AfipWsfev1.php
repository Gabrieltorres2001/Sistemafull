...
class AfipWsfev1
{
    ...
    public function FECAESolicitar($FeCAEReq, $Comprobante)
    {
        $prm = array(
                    'Auth'          => array(
                        'Token'     => $this->token,
        ...
        $results = $this->client->FECAESolicitar($prm, $Comprobante);
        if (!$this->getErrorsDos($results, 'FECAESolicitar', $Comprobante))
        {
    ...
    private function getErrors($results, $method)
    {
        ...
        file_put_contents(TMP_PATH.'CUIT_'.$this->cuit."_WSFEv1_request_".intval(date('YmdHis'))."_".$method.".xml",$this->client->__getLastRequest());
        file_put_contents(TMP_PATH.'CUIT_'.$this->cuit."_WSFEv1_response_".intval(date('YmdHis'))."_".$method.".xml",$this->client->__getLastResponse());
    ...
    private function getErrorsDos($results, $method, $Comprobante)
    {
        // Quitar comentarios para ver el log generado

        //file_put_contents(TMP_PATH.'CUIT_'.$this->cuit."_WSFEv1_request_".intval(date('Ymd'))."_".$method.".xml",$this->client->__getLastRequest());
        //file_put_contents(TMP_PATH.'CUIT_'.$this->cuit."_WSFEv1_response_".intval(date('Ymd'))."_".$method.".xml",$this->client->__getLastResponse());
		file_put_contents(TMP_PATH.'CUIT_'.$this->cuit."_WSFEv1_request_".intval(date('YmdHis'))."_".$Comprobante."_".$method.".xml",$this->client->__getLastRequest());
        file_put_contents(TMP_PATH.'CUIT_'.$this->cuit."_WSFEv1_response_".intval(date('YmdHis'))."_".$Comprobante."_".$method.".xml",$this->client->__getLastResponse());
		
		//Septiembre 2018. Pruebo guardar los errores de CAE en la base de datos.
		//Tengo: Cuit, Pregunta o respuesta, Fecha, Hora, Combrobante (numero tabla comprobantes), Metodo, XML
		//Creamos la conexión
		include_once '../../includes/sp_connect.php';
		$conexion_sp=mysqli_connect(HOSTSP,USERSP,PASSWORDSP,DATABASESP) or
			file_put_contents(TMP_PATH."WSFEv1_request_".intval(date('YmdHis'))."_".$Comprobante."_".$method.".xml","Problemas con la conexión al agregar item a wsfe");
			//die("Problemas con la conexión");
			mysqli_query($conexion_sp,"set names 'utf8'");
		//generamos la consulta de agregar registro en tabla WSFE.
		//Primero la pregunta
		if(!$resultactPreg = mysqli_query($conexion_sp, "insert into wsfe (Fecha, Hora, Cuit, Comunicacion, Comprobante, Metodo, XML) values ('".intval(date('Ymd'))."','".intval(date('His'))."', '".$this->cuit."', 'WSFEv1_request', '".$Comprobante."', '".$method."','".$this->client->__getLastRequest()."')")){
			 //die("Problemas con la consulta de agregar item al request a wsfe");
			 file_put_contents(TMP_PATH."WSFEv1_request_".intval(date('YmdHis'))."_".$Comprobante."_".$method.".xml","Problemas con la consulta de agregar item al request a wsfe".$resultactPreg);
		}		
		//segundo la respuesta
		if(!$resultactResp = mysqli_query($conexion_sp, "insert into wsfe (Fecha, Hora, Cuit, Comunicacion, Comprobante, Metodo, XML) values ('".intval(date('Ymd'))."','".intval(date('His'))."', '".$this->cuit."', 'WSFEv1_response', '".$Comprobante."', '".$method."','".$this->client->__getLastResponse()."')")){
			 //die("Problemas con la consulta de agregar item al response a wsfe");
			 file_put_contents(TMP_PATH."WSFEv1_response".intval(date('YmdHis'))."_".$Comprobante."_".$method.".xml","Problemas con la consulta de agregar item al response a wsfe".$resultactPreg);
		}			
