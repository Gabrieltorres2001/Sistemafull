...
echo "Comprobantes emitidos a este cliente:";
...
echo "<th width='4'>TC Facturado</th>"; 
//echo "<th width='4'></th>"; 
echo "</tr>";   
while ($rowFyF = mysqli_fetch_row($resultFondosyFacturas)){  
	//generamos la consulta DE BUSCAR LAS FACTURAS EMITIDAS
	if(!$resultDetalle = mysqli_query($conexion_sp, "select Id, NumeroFactura, TipoFactura, FechaFactura, ImporteTotal, IdComprobante, IdEnviado, CAE from caeafip where Id='".$rowFyF[0]."' order by Id")) die("Problemas con la consulta2");
	if ($row = mysqli_fetch_row($resultDetalle)){  	
		...
		if ($notaCredito==1){
				...
				if(!$tipoCambioUsado = mysqli_query($conexion_sp, "select tipoCambio from datosauxfacturasemitidas where CAE = '".$row[7]."' limit 1")) die("Problemas con la consulta forma de pago en datosauxfacturasemitidas");	
				$rowtipoCambioUsado = mysqli_fetch_array($tipoCambioUsado);
				echo "<td name='FilaFactura' id='$row[0]'>".$rowtipoCambioUsado["tipoCambio"]."</td>";
		}		
