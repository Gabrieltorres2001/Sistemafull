...
while ($rowFyF = mysqli_fetch_row($resultFondosyFacturas)){  
	...
	if ($row = mysqli_fetch_row($resultDetalle)){  	
		...
		if ($notaCredito==1){
			...
			//$Total=$Total-number_format($row[4],2,'.','');
			echo "<td name='FilaFactura' id='$row[0]'>€ -.--</td>";
			} elseif ($row[6]==1) {
				//La NC es en Dolares
				echo "<td name='FilaFactura' id='$row[0]'>USD -".number_format($row[4],2,'.',',')."</td>"; 
				//$Total=$Total-number_format($row[4],2,'.','');
				echo "<td name='FilaFactura' id='$row[0]'>USD -.--</td>";		
					} else {
						//La NC es en Pesos
						echo "<td name='FilaFactura' id='$row[0]'>$ -".number_format($row[4],2,'.',',')."</td>"; 
						//$Total=$Total-number_format($row[4],2,'.','');
		...
		//if ($notaCredito==0){echo "<td name='FilaFactura' id='$row[0]'><img name='AgregaPago' id='$row[0]&imagenAgregaPago' src='./images/Agregar.jpg' width='25' height='25'><img name='VerPagos' id='$row[0]&imagenVerPagos' src='./images/lupa.jpg' width='25' height='25'></td>";}
		echo "</tr>";	
	} 
}
echo "</table>";
