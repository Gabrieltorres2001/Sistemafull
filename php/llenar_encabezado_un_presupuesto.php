...
//generamos la consulta
   if(!$resultComprobante = mysqli_query($conexion_sp, "select NumeroComprobante, FechaComprobante, NonmbreEmpresa, ApellidoContacto, CondicionesPago, Notas, Confecciono, Transporte, PlazoEntrega, NumeroComprobante01, Solicito from comprobantes where IdComprobante='".$_REQUEST['idcomprobante']."' limit 1")) die("Problemas con la consulta1");
	...
	if ($soyYo==0) {
		...
		if ($puedoCotizar==0) {echo"<input type='button' id='asignarmePresup' value='Asignarme este presupuesto' disabled>";} else {echo"<input type='button' id='asignarmePresup' value='Asignarme este presupuesto'/>";}
		}
	echo"<br>";
	...
	{echo "<p style='border: 1px solid red;border-radius: 5px;' align=center> Este presupuesto no forma parte de ningún legajo electrónico</p>";}
