...
echo "<th width='125px' style='text-align:center'>Número(s) de serie</th>";
...
while ($row = mysqli_fetch_row($resultDetalle)){  
	...
	//Mio. Habilitado
		$colorFondo='#abf1ab';
		if ($art['EnStock']<=$art['StockMinimo']) {$colorFondo='yellow';}	
		if ($art['EnStock']<1) {$colorFondo='#FA5858';}
	...
	if ($soyYo==0) {
		...
		if ($row[16]==0){
			//Sin checkear
			echo "<td id='$row[0]&$row[2]&cumplido' align='center'><input name='xxxxt' id='$row[0]&$row[2]&chkcumplido' type='checkbox' Disabled ></input></td>";
			} else {
			//Checkeado
			echo "<td id='$row[0]&$row[2]&cumplido' align='center'><input name='xxxxt' id='$row[0]&$row[2]&chkcumplido' type='checkbox' Disabled checked></input></td>";
			}
		} else {
		// es mio
		if ($row[16]==0){
