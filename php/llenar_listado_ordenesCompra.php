...
//generamos la consulta
   if(!$result = mysqli_query($conexion_sp, "select NumeroComprobante,FechaComprobante,NonmbreEmpresa,IdComprobante from comprobantes where TipoComprobante=9 ORDER BY IdComprobante desc")) die("Problemas con la consulta1");

echo"<input type='button' id='listarTodos' value='Todos' Disabled>";
echo"<input type='button' id='listarMios' value='Mios'>";
echo"<br>";	
echo"<ul class='nav navbar-nav'>";
echo"<li>  Ordenes de compra ya realizadas:  </li>";
echo"</ul>";
echo "<br>";
...
while ($row = mysqli_fetch_array($result)){   
    ...
    echo "<td name='xxxxrt' id=".$row['IdComprobante'].">".$row['NumeroComprobante']."</td>";   
