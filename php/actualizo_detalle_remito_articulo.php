...
//LUEGO BUSCO EL VALOR DEL PRODUCTO EN LA TABLA DE PRODUCTOS
	if(!$resultLectProd = mysqli_query($conexion_sp, "select ValorVenta,MonedaOrigen from productos where IdProducto = '".$rowresultLect['IdProducto']."' limit 1")){
		echo"<label style='font-size:1em; font-weight:bold; color:red'>No actualizado</label>";
...
//LUEGO CALCULO EL VALOR DEL PRODUCTO PARA ESTE PRESUPUESTO EN FUNCION DE LOS DESCUENTOS
	$costoUnit=number_format($rowresultLectProd['ValorVenta'],2,'.','');
	$Subtot=(0)*number_format($costoUnit,2,'.','');
//generamos la consulta de actualizacion de los campos que cambian en consecuencia de los descuentos (costo unitario y subtotal)
	if(!$resultact = mysqli_query($conexion_sp, "update detallecomprobante set Descuento='0', desc1='0', desc2='0', Cantidad='0', CostoUnitario = '".$costoUnit."', Moneda='".$rowresultLectProd['MonedaOrigen']."', SubTotal = '".$Subtot."', actualiz = now() where IdDetalleComprobante = '".$_REQUEST['idcomprobante']."'")){
		echo"<label style='font-size:1em; font-weight:bold; color:red'>No actualizado</label>";
