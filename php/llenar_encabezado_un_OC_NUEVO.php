   ...
   if(!$resultComprobante = mysqli_query($conexion_sp, "select NumeroComprobante, FechaComprobante, NonmbreEmpresa, ApellidoContacto, CondicionesPago, Notas, Confecciono, Transporte, PlazoEntrega, NumeroComprobante01, Solicito from comprobantes where IdComprobante='".$_REQUEST['idcomprobante']."' limit 1")) die("Problemas con la consulta1");
	...
	echo"<select id='PlazoEntrega' name='PlazoEntrega'>";
		...
		while ($rowPlazoEnt = mysqli_fetch_array($plazoEnt)){
		  if ($rowPlazoEnt['PlazoEnDescripciontrega']==$reg['PlazoEntrega']){
	...
	echo"<select id='Transporte' name='Transporte'>";
		echo"<option value='0'> </option>";
		//echo"<option value='1' selected>Entrega en Neuquén Capital - Neuquén</option>";
	...
	echo"<input id='PeticionOferta' class='input' name='PeticionOferta' type='text' size='50' value='".$reg['NumeroComprobante01']."'>";
	echo"<br>";
	echo"<label for='Notas'>Notas:</label>";
	echo"<textarea id='Notas' class='input' overflow='scroll' name='Notas' resize='none' cols='100' rows='2'>".$reg['Notas']."</textarea>";
	echo"<br>";
	//echo"<input type='button' id='aceptarCambiaDatos' value='Aceptar'>";
	echo"<input id='ocEnviada' class='input' name='ocEnviada' type='text' size='20' style='width:140px;height:20px;background-color:red;color:yellow;font-size:10pt; font-family:Verdana;text-align:center;' value='OC NO GENERADA' readonly>";
