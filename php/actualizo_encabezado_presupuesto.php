...
//generamos la consulta de actualizacion
	if(!$resultact = mysqli_query($conexion_sp, "update comprobantes set CondicionesPago = '".$laFormaPago."', Notas = '".$_REQUEST['textNotas']."', Transporte = '".$elTransporte."', PlazoEntrega = '".$elPlazoEntrega."', CondicionesPago = '".$laFormaPago."', Solicito = '".$_REQUEST['solicitaa']."', NumeroComprobante01 = '".$_REQUEST['textPetOfer']."', actualiz = now() where (NumeroComprobante = '".$_REQUEST['numpresupuesto']."' and TipoComprobante='5')")){
		echo"Presupuesto NO actualizado";
		 die("Problemas con la consulta de actualizacion");
	}
	else {
		echo"<label style='font-size:1em; font-weight:bold; color:red'>Presupuesto actualizado</label>";
