...
//generamos la consulta
   if(!$result = mysqli_query($conexion_sp, "select NumeroComprobante,FechaComprobante,NonmbreEmpresa,IdComprobante from comprobantes where TipoComprobante=5 and Confecciono='".$_REQUEST['sesses']."' ORDER BY IdComprobante desc")) die("Problemas con la consulta1");

echo"<input type='button' id='listarTodos' value='Todos'>";
echo"<input type='button' id='listarMios' value='Mios' Disabled>";
echo"<br>";	
echo"<ul class='nav navbar-nav'>";
echo"<li>  Presupuestos MIOS ya realizados:  </li>";
echo"</ul>";
echo "<br>";
...
while ($row = mysqli_fetch_row($result)){   
    echo "<tr id=$row[3]>";  
    echo "<td name='xxxxrt' id=$row[3]>$row[0]</td>";   
