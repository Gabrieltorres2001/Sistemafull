...
//generamos la consulta
   if(!$resultComprobante = mysqli_query($conexion_sp, "select NumeroComprobante, FechaComprobante, NonmbreEmpresa, CondicionesPago, Notas, Confecciono, NumeroComprobante01, Solicito, NumeroComprobante02 from comprobantes where IdComprobante='".$_REQUEST['idcomprobante']."' limit 1")) die("Problemas con la consulta1");
	...
	if(!$resultEmp = mysqli_query($conexion_sp, "select Organizacion, Observaciones, CondDePago, CUIT, ActividEmpresa from organizaciones where id='".$reg['NonmbreEmpresa']."' limit 1")) die("Problemas con la consulta2");
