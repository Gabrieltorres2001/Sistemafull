...
//Primero busco en las tablas auxiliares los datos auxiliares de plazo entrega, forma de pago, responsable de solicito y transporte
		if(!$idComprobante = mysqli_query($conexion_sp, "select IdComprobante from comprobantes where (NumeroComprobante = '".$_REQUEST['numpresupuesto']."' and TipoComprobante='5') limit 1")) die("Problemas con la comprobantes");
...
//generamos la consulta de actualizacion
	if(!$resultact = mysqli_query($conexion_sp, "update comprobantes set CondicionesPago = '".$laFormaPago."', Notas = '".$_REQUEST['textNotas']."', Transporte = '".$elTransporte."', PlazoEntrega = '".$elPlazoEntrega."', CondicionesPago = '".$laFormaPago."', Solicito = '".$_REQUEST['solicitaa']."', NumeroComprobante01 = '".$_REQUEST['textPetOfer']."', NumeroComprobante02 = '".$_REQUEST['textPreimpreso']."', UsuarioModificacion = '".$_REQUEST['textNumFac']."', actualiz = now() where (NumeroComprobante = '".$_REQUEST['numremito']."' and TipoComprobante='3')")){
		echo"Remito NO actualizado";
		 die("Problemas con la consulta de actualizacion");
	}
	else {
		echo"<label style='font-size:1em; font-weight:bold; color:red'>Remito actualizado</label>";
