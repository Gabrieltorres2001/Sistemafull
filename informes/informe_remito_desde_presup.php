...
//============================================================+
	...
	//generamos la consulta para el encabezado
	   if(!$resultComprobante = mysqli_query($conexion_sp, "select NumeroComprobante, FechaComprobante, NonmbreEmpresa, ApellidoContacto, CondicionesPago, Notas, NumeroComprobante01, PlazoEntrega, Confecciono, MantiniemtoOferta, Transporte, Solicito, UsuarioModificacion, NumeroComprobante02, ConddeIva from comprobantes where TipoComprobante=3 and IdComprobante='".$_REQUEST['idrto']."' limit 1")) die("Problemas con la consulta1");
		...
		if(!$resultEmpDir = mysqli_query($conexion_sp, "select Direccion, Ciudad, Provoestado from direcciones where CUIT='".$regContEmp['idOrganizacion']."' order by id asc limit 1")) die("Problemas con la consulta2");
	...
	//generamos la consulta para el detalle	
		...
		if(!$resultDetalle = mysqli_query($conexion_sp, "select * from detallecomprobante where IdComprobante='".$_REQUEST['idrto']."' order by Orden")) die("Problemas con la consulta2");
//============================================================+
...
//============================================================+
	...
	public function Footer() {
		...
		$this->SetFont('helvetica', 'I', 8);
		$this->SetY(-35);
		$this->Cell(0, 10, 'Firma            Aclaración            Nº Legajo            Fecha', 0, false, 'R', 0, '', 0, false, 'T', 'M');
		$this->SetY(-14);
...
while ($row = mysqli_fetch_row($resultDetalle)){   
	if(!$resultArticulo = mysqli_query($conexion_sp, "select descricpcion, MonedaOrigen, ValorVenta, IVA, UnidadMedida, ComposicionyDescirpcion from productos where IdProducto='".$row[2]."' limit 1")) die("Problemas con la consulta2");
    ...
    $html = $html .'
	<tr>
		<td style="font-size:0.95em; font-weight:normal">'.$row[7].'</td>
		<td style="font-size:0.95em; font-weight:normal">'.$rowProd['descricpcion'].'</td>
		<td style="font-size:0.95em; font-weight:normal; text-align: right">'.$row[3].'</td>
	...
	//DETALLE DEL ARTICULO
	IF (($_REQUEST['serie']=='1') and ($row[5]!=NULL)){
	$html = $html .'<tr>
		<td style="font-size:0.95em; font-weight:normal">  </td>
		<td colspan="3" style="font-size:0.66em; font-weight:normal">Número de serie: '.$row[5].'</td>
		<td style="font-size:0.95em; font-weight:normal">  </td>
	</tr>';}
}	
