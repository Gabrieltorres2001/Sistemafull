...
//TEXTOS DEL HEADER
		...
		$this->Cell(1, 1, $this->decrTipoComprobante.' Nº 0000-00000000', 0, false, 'L', 0, '', 0, false, 'M', 'M');
...
//============================================================+
	...
	public function Footer() {
		...
		$txt='0000000000000000000000000000000000000000';
...
$pdf->numeroComprobante=str_pad($_REQUEST['NumeroFactura'], 8,"0", STR_PAD_LEFT);
$auxNF=(string)str_pad($_REQUEST['NumeroFactura'], 8,"0", STR_PAD_LEFT);
$pdf->CAE=$_REQUEST['numCAE'];
$pdf->vtoCAE=str_replace("-","", $_REQUEST['VtoCAE']);
...
while ($rowDetalle = mysqli_fetch_array($resultDetalle)){   
	...
	<tr>
		...
		IF ($_REQUEST['tipoFactura']=='A'){
			// IVA discriminado
			$html = $html .'
				<td style="font-size:0.95em; font-weight:normal; text-align: right">'.$simboloMoneda.' '.number_format($rowDetalle['CostoUnitario']*$_REQUEST['tipoCambio'],2,',','.').'</td>
				<td style="font-size:0.95em; font-weight:normal; text-align: right">'.$simboloMoneda.' '.number_format($rowDetalle['SubTotal']*$_REQUEST['tipoCambio'],2,',','.').'</td>
				<td style="font-size:0.9em; font-weight:normal; text-align: right"><small>+'.$rowIVA['Texto'].'</small></td>
			</tr>';}
			else {//$_REQUEST['tipoFactura']=='B'
				//IVA incluido
				$html = $html .'
						<td style="font-size:0.95em; font-weight:normal; text-align: right">'.$simboloMoneda.' '.number_format($rowDetalle['CostoUnitario']*(1+$rowIVA['Valor'])*$_REQUEST['tipoCambio'],2,',','.').'</td>
						<td style="font-size:0.95em; font-weight:normal; text-align: right">'.$simboloMoneda.' '.number_format($rowDetalle['SubTotal']*(1+$rowIVA['Valor'])*$_REQUEST['tipoCambio'],2,',','.').'</td>
					</tr>';}
	//separo los ivas
	if ($rowProd['IVA']=='1'){
		$subtotaliva21=$subtotaliva21+($rowDetalle['SubTotal']*$_REQUEST['tipoCambio']);
		$IVAdel21=$IVAdel21+($rowDetalle['SubTotal']*($rowIVA['Valor'])*$_REQUEST['tipoCambio']);}
	if ($rowProd['IVA']=='2'){
		$subtotaliva10=$subtotaliva10+($rowDetalle['SubTotal']*$_REQUEST['tipoCambio']);
		$IVAdel10=$IVAdel10+($rowDetalle['SubTotal']*($rowIVA['Valor'])*$_REQUEST['tipoCambio']);}
	...
	//	else {//$_REQUEST['tipoFactura']=='B'
			$totalPresup= $totalPresup + ($rowDetalle['SubTotal']*(1+$rowIVA['Valor'])*$_REQUEST['tipoCambio']);
	//}
...
<table border="0" {border-collapse: collapse;}>
	<tr>
		<td width="100%" style="font-size:0.85em; text-align: left">NOTA: Operación concertada en dólares estadounidenses billete/Euro billete. El monto en pesos facturado equivale a  '.$rowmonedaGeneral['Simbolo'].' '.number_format(($totalPresup/$_REQUEST['tipoCambio']),2,',','.').'.</td>
	</tr>
	<tr>
		<td width="100%" style="font-size:0.85em; text-align: left">Tipo de cambio aplicado: ($ / '.$rowmonedaGeneral['Simbolo'].') '.number_format($_REQUEST['tipoCambio'],2,',','.').'. Los valores recibidos en concepto de pago de la presente, serán considerados como '.$rowmonedaGeneral['Origen'].' al tipo de cambio vigente al día anterior de efectivo el pago.</td>
	</tr>
	<tr>
...
// draw jpeg image
$pdf->Image('simulacion.jpg', 20, 100, 160, 50, '', '', '', true, 72);
...
if (file_exists($rowresultRutaPresup['ContenidoValor'].$regEmp['Organizacion'].'\\')== true) {
		$pdf->Output($rowresultRutaPresup['ContenidoValor'].$regEmp['Organizacion'].'\\('.$_REQUEST['TipoCompFactura'].$auxNF.')'.$regEmp['Organizacion'].' '.$descripTipoComprobante.' ('.$reg['NumeroComprobante'].')'.date("dmy").'.pdf', 'I');}
	else {
		$pdf->Output($rowresultRutaPresup['ContenidoValor'].'('.$_REQUEST['TipoCompFactura'].$auxNF.')'.$regEmp['Organizacion'].' '.$descripTipoComprobante.' ('.$reg['NumeroComprobante'].')'.date("dmy").'.pdf', 'I');
	};
//============================================================+
// END OF FILE
//============================================================+
