...
//============================================================+
	...
	//generamos la consulta para los datos de CAEAFIP (Tiene que ser order id desc)
		if(!$resultAfip = mysqli_query($conexion_sp, "select NumeroFactura, VtoCAE, FechaFactura from caeafip where CAE='".$_REQUEST['numCAE']."' limit 1")) die("Problemas con la consulta caeafip");
		$regresultAfip = mysqli_fetch_array($resultAfip);	
	
	//generamos la consulta para la moneda ORIGINAL de toda la factura	
	if(!$monedaGeneral = mysqli_query($conexion_sp, "select Simbolo, Origen from monedaorigen where IdRegistroCambio='".$regresultDAFE['monedaOriginal']."' limit 1")) die("Problemas con la consulta monedaorigen");
...
while ($rowDetalle = mysqli_fetch_array($resultDetalle)){   
	...
	<tr>
		...
		IF ($regresultDAFE['tipoFactura']=='A'){
			// IVA discriminado
			$html = $html .'
				<td style="font-size:0.95em; font-weight:normal; text-align: right">'.$simboloMoneda.' '.number_format($rowDetalle['CostoUnitario']*$regresultDAFE['tipoCambio'],2,',','.').'</td>
				<td style="font-size:0.95em; font-weight:normal; text-align: right">'.$simboloMoneda.' '.number_format($rowDetalle['SubTotal']*$regresultDAFE['tipoCambio'],2,',','.').'</td>
				<td style="font-size:0.9em; font-weight:normal; text-align: right"><small>+'.$rowIVA['Texto'].'</small></td>
			</tr>';}
			else {//$regresultDAFE['tipoFactura']=='B'
				//IVA incluido
				$html = $html .'
						<td style="font-size:0.95em; font-weight:normal; text-align: right">'.$simboloMoneda.' '.number_format($rowDetalle['CostoUnitario']*(1+$rowIVA['Valor'])*$regresultDAFE['tipoCambio'],2,',','.').'</td>
						<td style="font-size:0.95em; font-weight:normal; text-align: right">'.$simboloMoneda.' '.number_format($rowDetalle['SubTotal']*(1+$rowIVA['Valor'])*$regresultDAFE['tipoCambio'],2,',','.').'</td>
					</tr>';}
	//separo los ivas
	if ($rowProd['IVA']=='1'){
		$subtotaliva21=$subtotaliva21+($rowDetalle['SubTotal']*$regresultDAFE['tipoCambio']);
		$IVAdel21=$IVAdel21+($rowDetalle[6]*($rowIVA['Valor'])*$regresultDAFE['tipoCambio']);}
	if ($rowProd['IVA']=='2'){
		$subtotaliva10=$subtotaliva10+($rowDetalle['SubTotal']*$regresultDAFE['tipoCambio']);
		$IVAdel10=$IVAdel10+($rowDetalle['SubTotal']*($rowIVA['Valor'])*$regresultDAFE['tipoCambio']);}
	...
	//	else {//$regresultDAFE['tipoFactura']=='B'
			$totalPresup= $totalPresup + ($rowDetalle['SubTotal']*(1+$rowIVA['Valor'])*$regresultDAFE['tipoCambio']);
	//}
...
<table border="0" {border-collapse: collapse;}>
	<tr>
		<td width="100%" style="font-size:0.85em; text-align: left">NOTA: Operación concertada en dólares estadounidenses billete/Euro billete. El monto en pesos facturado equivale a  '.$rowmonedaGeneral['Simbolo'].' '.number_format(($totalPresup/$regresultDAFE['tipoCambio']),2,',','.').'.</td>
	</tr>
	<tr>
		<td width="100%" style="font-size:0.85em; text-align: left">Tipo de cambio aplicado: ($ / '.$rowmonedaGeneral['Simbolo'].') '.number_format($regresultDAFE['tipoCambio'],2,',','.').'. Los valores recibidos en concepto de pago de la presente, serán considerados como '.$rowmonedaGeneral['Origen'].' al tipo de cambio vigente al día anterior de efectivo el pago.</td>
	</tr>
	<tr>
