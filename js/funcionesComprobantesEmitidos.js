...
function mostrarDetalles(celda){
  ...
  numeroMenu=celda.target.id;
	...
	var anno=numeroMenu.substring(0, 4);
	var mes=numeroMenu.substring(4);
	//alert(anno+","+mes);
  conexion2=new XMLHttpRequest(); 
  conexion2.onreadystatechange = procesarEventos2;
  var aleatorio=Math.random();
  var obnn=document.getElementById('numberses').value;
  conexion2.open('GET','./php/detallesmesfacturado.php?aanno='+anno+"&mmes="+mes+"&rnadom="+aleatorio+"&sesses="+obnn, true);
  conexion2.send();
...
function procesarEventos2()
{
	...
	if(conexion2.status == 200)
		{ 
			document.getElementById('ComprobantesEmitidosDetalles').innerHTML=conexion2.responseText; 	  
		}
  } 
}
